...
class Test09IPFamilyInconsistent(StandardStructureTestBase):
    """测试 9_IPFamily校验不一致"""

    def test_01_fault_count(self):
        """测试故障数量"""
        fault_groups = self._analyze_test_case("9_IPFamily校验不一致")
        self._assert_fault_count(fault_groups, 1)

    def test_02_fault_name(self):
        """测试故障名称"""
        fault_groups = self._analyze_test_case("9_IPFamily校验不一致")
        self._assert_fault_exists(fault_groups, "IP Family校验不一致")
    ...
    def test_04_comm_info(self):
        ...
        test_path = PROJECT_ROOT / "test_data" / "标准目录结构" / "9_IPFamily校验不一致" / "log1"
        worker_id = str(test_path.absolute())

        self._assert_comm_info(
            fault_groups,
            fault_name="IP Family校验不一致",
            expected_process_id="144905",
