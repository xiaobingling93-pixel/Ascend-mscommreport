...
class Test10TlsConfigInconsistent(StandardStructureTestBase):
    """测试 10_TLS配置不一致"""

    def test_01_fault_count(self):
        """测试故障数量"""
        fault_groups = self._analyze_test_case("10_TLS配置不一致")
        self._assert_fault_count(fault_groups, 1)

    def test_02_fault_name(self):
        """测试故障名称"""
        fault_groups = self._analyze_test_case("10_TLS配置不一致")
        self._assert_fault_exists(fault_groups, "TLS信息配置不一致")
    ...
    def test_03_solution(self):
        ...
        self._assert_solution_contains(
            fault_groups,
            fault_name="TLS信息配置不一致",
            expected_content="TLS"
        )
    ...
    def test_04_comm_info(self):
        ...
        test_path = PROJECT_ROOT / "test_data" / "标准目录结构" / "10_TLS配置不一致" / "log1"
        worker_id = str(test_path.absolute())

        self._assert_comm_info(
            fault_groups,
            fault_name="TLS信息配置不一致",
            expected_process_id="94774",
