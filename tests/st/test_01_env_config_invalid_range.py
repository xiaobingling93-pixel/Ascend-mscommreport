...
class Test01EnvConfigInvalidRange(StandardStructureTestBase):
    """测试 1_环境变量配置异常超阈值"""

    def test_01_fault_count(self):
        """测试故障数量"""
        fault_groups = self._analyze_test_case("1_环境变量配置异常超阈值")
        self._assert_fault_count(fault_groups, 1)

    def test_02_fault_name(self):
        """测试故障名称"""
        fault_groups = self._analyze_test_case("1_环境变量配置异常超阈值")
        self._assert_fault_exists(fault_groups, "环境变量配置不符合取值范围")
    ...
    def test_03_solution(self):
        ...
        self._assert_solution_contains(
            fault_groups,
            fault_name="环境变量配置不符合取值范围",
            expected_content="环境变量"
        )
    ...
    def test_04_comm_info(self):
        ...
        test_path = PROJECT_ROOT / "test_data" / "标准目录结构" / "1_环境变量配置异常超阈值" / "log1"
        worker_id = str(test_path.absolute())

        self._assert_comm_info(
            fault_groups,
            fault_name="环境变量配置不符合取值范围",
            expected_process_id="7988",
