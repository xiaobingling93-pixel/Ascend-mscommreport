...
"""
测试 42_参数面建链超时client进程提前退出

测试场景：建链超时，MyRole为server，client端进程提前退出
"""
from pathlib import Path
from tests.st.base import StandardStructureTestBase, PROJECT_ROOT


class Test42ClientProcessExit(StandardStructureTestBase):
    """测试 42_参数面建链超时client进程提前退出"""

    def test_01_fault_count(self):
        """测试故障数量：param_plane_link_establish_timeout去重后应该有1个故障"""
        fault_groups = self._analyze_test_case("42_参数面建链超时client进程提前退出")
    ...
    def test_03_solution_client_process_exit(self):
        """测试解决方案：应该提示client端进程提前退出"""
        fault_groups = self._analyze_test_case("42_参数面建链超时client进程提前退出")

        self._assert_solution_contains(
            fault_groups,
            fault_name="参数面建链超时",
            expected_content="rank[0]作为client端所在的进程提前退出"
        )
    ...
    def test_05_solution_contains_contact_expert(self):
        ...
        self._assert_solution_contains(
            fault_groups,
            fault_name="参数面建链超时",
            expected_content="请联系HCCL专家排查client端提前退出的原因"
        )

    def test_06_comm_info(self):
        """测试通信域创建信息"""
        fault_groups = self._analyze_test_case("42_参数面建链超时client进程提前退出")

        # 计算worker_id：log4目录的绝对路径
        test_path = PROJECT_ROOT / "test_data" / "标准目录结构" / "42_参数面建链超时client进程提前退出" / "log4"
