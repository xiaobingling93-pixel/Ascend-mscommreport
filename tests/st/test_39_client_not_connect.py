...
"""
测试 39_client超时Server未发起connect

测试场景：建链超时，client端没有发起socket connect
"""
from pathlib import Path
from tests.st.base import StandardStructureTestBase, PROJECT_ROOT


class Test39ClientNotConnect(StandardStructureTestBase):
    """测试 39_client超时Server未发起connect"""
    ...
    def test_03_solution_client_not_connect(self):
        """测试解决方案：应该提示client作为client超时"""
        fault_groups = self._analyze_test_case("39_client超时Server未发起connect")
    ...
    def test_04_solution_contains_entry_log_suggestion(self):
    ...
    def test_05_solution_contains_entry_count_table(self):
        """测试解决方案：应该包含通信算子执行次数统计"""
        fault_groups = self._analyze_test_case("39_client超时Server未发起connect")

        self._assert_solution_contains(
            fault_groups,
            fault_name="参数面建链超时",
            expected_content="通信算子执行次数统计如下"
        )
    ...
    def test_08_comm_info(self):
        ...
        test_path = PROJECT_ROOT / "test_data" / "标准目录结构" / "39_client超时Server未发起connect" / "log1"
