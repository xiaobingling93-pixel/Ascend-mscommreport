...
class FaultGroupChecker:
    ...
    def get_unconnected_rank_ids(
        ...
        """
        获取未连接到 server 节点的 rankId 列表

        如果 log_text 存在，使用 RankIdExtractor 从中提取 connected rankinfo，
        计算 all_ranks - connected_ranks 得到未连接的 rankId。
        如果 log_text 不存在，所有 rank 都视为未连接。

        Args:
            context: 故障分析上下文
            identifier: 通信域标识符
            process_id: 当前进程号
            comm_info: 通信域信息（用于获取 total_ranks）
            log_text: 日志文本（可选）

        Returns:
            未连接的 rankId 列表，如果无法获取则返回 None
        """
        from ..collectors import RankIdExtractor
        ...
        # 没有日志文本，所有 rank 都视为未连接
        total_ranks = comm_info.ranks
        return list(range(total_ranks)) if total_ranks else None
